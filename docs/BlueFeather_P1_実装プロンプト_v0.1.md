# BlueFeather P1 実装プロンプト（Claude Code 用） v0.1
...
- 環境変数を読む設定アクセサを実装する：
  ...
  - `BLUEFEATHER_DB_PATH`（任意。既定 `bluefeather.db`）。
...
変更内容：
- `app/db/schema.sql` を読んでDB（`BLUEFEATHER_DB_PATH`）にテーブルを作成する。
...
python -c "import sqlite3,os; c=sqlite3.connect(os.environ.get('BLUEFEATHER_DB_PATH','bluefeather.db')); print('phases=', c.execute('select count(*) from phases').fetchone()[0]); print('items=', c.execute('select count(*) from rubric_items').fetchone()[0])"
